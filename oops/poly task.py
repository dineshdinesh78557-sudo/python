# Task 1: Vehicle Info System

# Base Class: Vehicle â†’ attributes: brand, model, year
# Child Classes:

# Car â†’ adds seating_capacity

# Bike â†’ adds engine_cc

# ðŸŽ¯ Goal: Create objects for both Car and Bike and print their details.


class parent:


    def __init__(self,brand,model,year):
        self.brand=brand
        self.model=model
        self.year=year

    def __str__(self):
        return f'hero {self.brand}' 
    def bikedetails():
        return f'toyoto,splendar'   

class child:

    def __init__(self,brand,model,year):
        self.brand=brand
        self.model=model
        self.year=year

    def __str__(self):
        return f' {self.brand} is {self.year}'
    
    def bikedetails(self):
        return f'car.bike'

st1=child('honda','omni','2026') 
print(st1)
print(st1.bikedetails())   


# ðŸ”¹ Task 2: School Management

# Base Class: Person â†’ attributes: name, age
# Child Classes:

# Student â†’ adds roll_number, grade

# Teacher â†’ adds subject, salary

class person:

    def __init__(self,name,age,rollno,grade):
        self.name=name
        self.age=age
        self.rollno=rollno
        self.grade=grade

    def __str__(self):
        return f'23 {self.rollno} a {self.grade}'
    def roll_number(self):
        return f'roll no,grade'
    
class student:

    def __init__(self,name,age,rollno,salary,grade):
        self.name=name
        self.rollno=rollno
        self.age=age
        self.salary=salary
        self.grade=grade
    def __str__(self):
        return f'  {self.rollno} is {self.grade}' 
    def roll_number():
             return f'name salary'
st=student('dinesh',22,23,20000,'A') 
print(st)   
  


# ðŸ”¹ Task 3: Online Shopping

# Base Class: Product â†’ attributes: name, price
# Child Classes:

# Electronics â†’ adds warranty_years

# Clothing â†’ adds size, material

# ðŸŽ¯ Goal: Create objects for each category and print their product details.  


class parant:
     
     def __init__(self,name,price,year):
          self.name=name
          self.price=price
          self.year=year
     def __str__(self):
          return f"1000 {self.price}"  
     def warrenty_years(self):
          return f"20 years----{self.year}"
     
class child:

     def __init__(self,name,price,year,size):
          self.name=name
          self.price=price
          self.year=year
          self.size=size
     def  __str__(self):
          return f"{self.name} {self.year} {self.size}" 
     def warrenty_year(self):
          return f"10 1000"  

st1=child('dinesh',1000,20,10) 
print(st1) 
print(st1.warrenty_year())    


# ðŸ”¹ Task 4: Banking

# Base Class: Account â†’ attributes: account_number, balance
# Child Classes:

# SavingsAccount â†’ adds interest_rate

# FixedDeposit â†’ adds duration_years

# ðŸŽ¯ Goal: Create objects for both account types and print all details.


class parant:


    def __init__(self,accno,balance,interest):
        self.accno=accno
        self.balance=balance
        self.interest=interest
    def __str__(self):
        return f'100 {self.accno}' 
    def interest_rate(self):
        return f'200 {self.interest}'

class child:
    def __init__(self,accno,balance,interest,year):
        self.accono=accno
        self.balance=balance
        self.interest=interest
        self.year=year
    def __str__(self):
        return f'{self.accono} {self.interest} {self.year}' 
    def duration_years(self):
        return f'100,10'  
st1=child(100,1000,200,10)   
print(st1) 
print(st1.duration_years())  


# # ðŸ”¹ Task 5: Library Management

# # Base Class: Book â†’ attributes: title, author
# # Child Classes:

# # FictionBook â†’ adds genre

# # Magazine â†’ adds issue_month

# # ðŸŽ¯ Goal: Create objects for both classes and show their details.
               
class parant: 

    def __init__(self,title,author,book):
        self.title=title
        self.author=author
        self.book=book
    def __str__(self):
        return f'python {self.book}' 
    def issue_month(self):
        return f'marsh {self.author}'  
class child:


    def __init__(self,title,author,book,month):
        self.title=title
        self.author=author
        self.book=book
        self.month=month
    def __str__(self):
        return f'  {self.title}  {self.month}  {self.author}'
    def issue_month(self):
        return f'python india marsh'
st1=child('india','marsh','python',2025)
print(st1)  
print(st1.issue_month())    
      
             
              

          
          
            
    
      
        
            
        

        
            
            
        

